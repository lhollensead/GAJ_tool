  ...
  output$Order <- renderUI({
    if(input$dataset == "none" ){#& input$survey=="old"|input$dataset == "none" & input$survey=="new"){
      sliderInput(inputId = "Order",
                  ...
                  min = as.Date('2020-08-01', origin='1970-01-01'), max = as.Date('2020-12-31', origin='1970-01-01'), value = c(as.Date('2020-08-01', origin='1970-01-01'),as.Date('2020-10-31', origin='1970-01-01')),
  ...
  output$valuebox2<-renderValueBox({
    if(input$dataset=="none"){#& input$survey=="old"){
      valueBox(
        ...
        value=nrow(spnum()), icon=icon("calendar-alt"))} 
    # else if(input$dataset=="none"& input$survey=="new"){
    #       valueBox(
    #         "Days in spring season",
    #         value=nrow(fes_spnum()), icon=icon("calendar-alt"))} 
    else
          if(input$dataset=="two"){#& input$survey=="old"){
            valueBox(
